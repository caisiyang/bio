...
const App: React.FC = () => {
  ...
  // Load from local storage on mount (for persistence between reloads if Gist isn't used immediately)
  useEffect(() => {
    const savedConfig = localStorage.getItem('neu_gist_config');
    if (savedConfig) {
      setGistConfig(JSON.parse(savedConfig));
    }
  }, []);
    ...
    <div className="min-h-screen w-full flex flex-col items-center py-10 px-4 sm:px-0 font-sans text-text-main relative">
      ...
      <div className="w-full max-w-sm flex flex-col items-center">
        ...
        <NeuCard className="w-full p-6 flex flex-col items-center text-center mb-8">
          ...
          <a href={profile.contactUrl} className="w-full block">
            <NeuButton className="w-full py-4 text-text-main font-bold text-sm tracking-wide">
              {profile.contactText}
            </NeuButton>
          </a>
        </NeuCard>
        ...
        <div className="grid grid-cols-4 gap-4 w-full mb-10">
          {profile.socials.map((social) => (
            <a key={social.id} href={social.url} target="_blank" rel="noopener noreferrer">
              <NeuButton className="w-full aspect-square rounded-2xl text-xl text-text-main hover:text-gray-800">
                <IconDisplay type={social.platform} />
              </NeuButton>
            </a>
          ))}
        </div>
            ...
            {profile.projects.map((project) => (
              <a key={project.id} href={project.link || '#'} target="_blank" rel="noopener noreferrer" className="block">
                <NeuCard className="p-3 h-full flex flex-col transition-transform hover:-translate-y-1">
                  ...
                  <h3 className="font-semibold text-xs text-text-main leading-tight mt-auto">
                    {project.title}
                  </h3>
                </NeuCard>
              </a>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <NeuButton 
          ...
          className="w-12 h-12 rounded-full flex items-center justify-center text-text-sub hover:text-text-main"
        >
          <FaCog size={20} />
        </NeuButton>
      </div>
